...
namespace TileContent
{
    ...
    public class TileLayerWriter : ContentTypeWriter<TileLayerContent>
    {
        protected override void Write(ContentWriter output, TileLayerContent value)
        {
            ...
            output.Write(value.TreasureChests.Count);
            foreach (TileLayerTreasureChests chest in value.TreasureChests)
            {
                output.Write(chest.LocationX);
                output.Write(chest.LocationY);
                output.Write(chest.Type);
                output.Write(chest.Item);
                output.Write(chest.Quantity);
            }

            output.Write(value.Doors.Count);
            foreach (TileLayerDoors door in value.Doors)
            {
                output.Write(door.LocationX);
                output.Write(door.LocationY);
            }
        }
